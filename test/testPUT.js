...
vows.describe('What the server sees with PUT operations').addBatch(
{
    'An empty put' : {
        topic : function(){
            rest.put(webServer(this.callback));
        },
        'will PUT /.' : function(request, response) {
    ...
    'A put to /asdf' : {
        topic : function(){
            rest.put(webServer(this.callback) + '/asdf');
        },
        'will PUT /asdf.' : function(request, response) {
            assert.strictEqual(request.method, 'PUT');
            assert.strictEqual(request.url, '/asdf');
        }
    },
    'A put with paramaters in the uri' : {
        topic : function(){
            rest.put(webServer(this.callback) + '/asdf?param1=one&param2=two');
        },
        'will pass the paramaters.' : function(request, response) {
            assert.strictEqual(request.url, '/asdf?param1=one&param2=two');
        }
    },
    'A put with paramaters in options.query' : {
        topic : function(){
            rest.put(webServer(this.callback) + '/asdf',{
                'query' :{
                    'param1' : 'one',
    ...
    'A put with paramaters in options.headers' : {
        topic : function(){
            rest.put(webServer(this.callback) + '/asdf',{
                'headers' :{
                    'param1' : 'one',
    ...
    'A put with overriding paramaters in options.headers' : {
        topic : function(){
            rest.put(webServer(this.callback) + '/asdf',{
                'headers' :{
    ...
    'A put with options.username and options.password' : {
        topic : function(){
            rest.put(webServer(this.callback) + '/asdf',{
                'username' : 'danwrong',
                'password' : 'flange'
    ...
    'A put with auth in the URL' : {
        topic : function(){
            rest.put((webServer(this.callback) + '/asdf').replace(
                'http://', 
    ...
    'A put with options.data === String' : {
        topic : function(){
            var self = this;
            rest.put(webServer(function(request){
                var data = '';
    ...
    'A put with options.data === Object' : {
        topic : function(){
            var self = this;
            rest.put(webServer(function(request){
                var data = '';
    ...
    'A put with simple multipart data' : {
        topic : function(){
            var self = this;
            rest.put(webServer(function(request){
                var data = '';
