...
vows.describe('What the server sees with DELETE operations').addBatch(
{
    'An empty del' : {
        topic : function(){
            rest.del(webServer(this.callback));
        },
        'will DELETE /.' : function(request, response) {
    ...
    'A del to /asdf' : {
        topic : function(){
            rest.del(webServer(this.callback) + '/asdf');
        },
        'will DELETE /asdf.' : function(request, response) {
            assert.strictEqual(request.method, 'DELETE');
            assert.strictEqual(request.url, '/asdf');
        }
    },
    'A del with paramaters in the uri' : {
        topic : function(){
            rest.del(webServer(this.callback) + '/asdf?param1=one&param2=two');
        },
        'will pass the paramaters.' : function(request, response) {
            assert.strictEqual(request.url, '/asdf?param1=one&param2=two');
        }
    },
    'A del with paramaters in options.query' : {
        topic : function(){
            rest.del(webServer(this.callback) + '/asdf',{
                'query' :{
                    'param1' : 'one',
    ...
    'A del with paramaters in options.headers' : {
        topic : function(){
            rest.del(webServer(this.callback) + '/asdf',{
                'headers' :{
                    'param1' : 'one',
    ...
    'A del with overriding paramaters in options.headers' : {
        topic : function(){
            rest.del(webServer(this.callback) + '/asdf',{
                'headers' :{
    ...
    'A del with options.username and options.password' : {
        topic : function(){
            rest.del(webServer(this.callback) + '/asdf',{
                'username' : 'danwrong',
                'password' : 'flange'
    ...
    'A del with auth in the URL' : {
        topic : function(){
            rest.del((webServer(this.callback) + '/asdf').replace(
                'http://', 
    ...
    'A delete with options.data === String' : {
        topic : function(){
            var self = this;
            rest.del(webServer(function(request){
                var data = '';
    ...
    'A delete with options.data === Object' : {
        topic : function(){
            var self = this;
            rest.del(webServer(function(request){
                var data = '';
